...
if (plot_distinct) {
  for (i in seq_along(tbl_l$distinct)) {
    ...
    height = length(unique(dat$var20)) / 4
    # header
...
```{r plot-OLD, echo=FALSE, fig.height=7, fig.width=15, dpi=72, message=FALSE, warning=FALSE, results='asis'}
# if (plot_distinct) {
#   for (i in seq_along(tbl_l$distinct)) {
#     dat = tbl_l$distinct[[i]]
#     col = names(tbl_l$distinct)[i]
...
#     cat('  \n###', col, '\n') # OPTION pander::pandoc.header(paste0('Column: ', col), level = 3)
#     # plots
#     print(pl_gg(dat = dat, x = 'var20', y = 'n_distinct', plot_type = plot_type))
#     cat('<a href="#top">Back to top</a>')
#     cat('  \n') # OPTION # pander::pandoc.p(''); pander::pandoc.p('')
#   }
# } else {
